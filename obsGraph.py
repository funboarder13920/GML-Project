...
def weak_nodes(G):
    ...
    for node in list(G.nodes()):
        if node in G.predecessors(node):
            continue
        if len(list(G.predecessors(node))) != n-1:
...
def dom_set(G):
    """Computes an approximation of the domination set of G"""
    return dominating_set.min_edge_dominating_set(G)


def dom_number(G):
    """Computes an approximation of the domination number of G"""
    return len(dom_set(G))
